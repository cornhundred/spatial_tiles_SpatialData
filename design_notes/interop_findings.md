...
what the protocol notes originally asserted. Each is recorded with how it was verified,
...
`display_xy` is written as `fixed_size_list<uint32>[2]` and `display_geometry` as
`list<list<fixed_size_list<uint32>[2]>>`, but **Parquet stores both as plain `List`**. The
...
|---|---|
| parquet-wasm | 0.7.1, 0.7.2 (latest) |
...
These are fetched from fixed paths relative to `base_url`. A missing or mis-shaped one
...
| `micron_to_image_transform.csv` | the real micron→pixel affine | scale bar wrong by the pixel size |
...
DegaFiles tiles are nearly black by design; the slider does the brightening. The profile
